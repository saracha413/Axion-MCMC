...
if __name__ == '__main__':
    ...
    gr_test = 50
    counter = 1
    steps_per_it = 50 #number of steps in MCMC before checking G-R test
    ...
    while gr_test > 1.2:
        ...
        best_vals = np.zeros((num_walkers, 5))
        ...
        for n in range(num_walkers):
            ...
            H0_list.append(H0)
            Djs_list.append(Djs)
            best_vals[i] = np.array([log10_axion_ac[len(log10_axion_ac)-1], log10_fraction_ac[len(log10_fraction_ac)-1], omega_cdm[len(omega_cdm) - 1], H0[len(H0) - 1]])
        
        num_steps = steps_per_it * counter
        #perform G-R test on all params
        gr_log10_axion_ax = grubin(log10_axion_ac_list, 5, num_walkers, num_steps)
        gr_log10_fraction_ac = grubin(log10_fraction_ac_list, 5, num_walkers, num_steps)
        gr_omega_cdm = grubin(omega_cdm_list, 5, num_walkers, num_steps)
        gr_omega_cdm = grubin(H0_list, 5, num_walkers, num_steps)
